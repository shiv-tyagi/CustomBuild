...
class BuildArtifactsCleaner:
    ...
    def __stale_artifacts_path_list(self) -> list:
        ...
        logger.debug(f"Scanning directory: {dir_to_scan} for stale artifacts")
        all_build_ids = bm.get_singleton().get_all_build_ids()
        logger.debug(f"Retrieved all build IDs: {all_build_ids}")

        dirs_to_keep = [
            pathlib.Path(bm.get_singleton().get_build_artifacts_dir(build_id))
        ...
        for f in dir_to_scan.iterdir():
            ...
            f_is_relative_to_any = any(
                f.is_relative_to(dir)
                for dir in dirs_to_keep
            )
            if not f_is_relative_to_any:
                stale_artifacts.append(str(f))

        logger.debug(f"Stale artifacts found: {stale_artifacts}")
    ...
    def __run(self) -> None:
        ...
        for path in self.__stale_artifacts_path_list():
            logger.info(f"Removing stale artifacts at {path}")
