...
class BuildProgressUpdater:
    ...
    def start(self) -> None:
        ...
        """
        logger.info("Starting BuildProgressUpdater.")
        self.__runner.start()
    ...
    def __generate_status_json(self) -> None:
        ...
        logger.debug(f"All build ids sorted: {all_build_ids_sorted}")
        ...
        for build_id in all_build_ids_sorted:
            ...
            logger.debug(f"Build info json: {bi_json}")
            builds_dict[build_id] = bi_json
        logger.debug(f"Builds dict: {builds_dict}")
