...
class TaskRunner:
    ...
    def __init__(self, tasks: tuple[tuple]) -> None:
        ...
        logger.info("Started task runner thread")
    ...
    def __run(self) -> None:
        ...
        while not self.__stop_event.is_set():
            ...
            time_to_next_call = min(next_call_times) - now
            logger.debug(
                f"Time to next call: {time_to_next_call} sec, "
                f"Runner id: {id(self)}"
            )
            if time_to_next_call > 0:
                self.__stop_event.wait(time_to_next_call)
