...
const TYPE_ORDER: Record<Version['type'], number> = { stable: 0, beta: 1, latest: 2, tag: 3, custom: 4 };
...
function versionBadge(t: Version['type']) {
    return ({
        stable: 'badge-green', beta: 'badge-yellow', latest: 'badge-blue', tag: 'badge-gray', custom: 'badge-gray',
    } as const)[t];
}
...
export function BuildForm({ initialConfig, onConsumeInitialConfig }: BuildFormProps) {
    ...
    const reset = useCallback(() => {
        ...
        setFeaturesLoading(false); setVehiclesLoading(false);
        setSubmitting(false);
        setError(null);
    ...
    async function startBuild() {
        ...
        try {
            ...
            if (gen === loadGenRef.current) setSubmitting(false);
        }
    }
