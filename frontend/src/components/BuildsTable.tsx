...
export function BuildsTable({ onRebuild }: BuildsTableProps = {}) {
    ...
    function doFetch(p: number, isInitial = false) {
        ...
        fetchBuilds(PAGE_SIZE + 1, p * PAGE_SIZE)
            ...
            .catch(() => {
                if (gen !== fetchGenRef.current) return;
                setLoadError('Failed to load builds');
            })
            .finally(() => {
                    ...
                    <div className="overflow-x-auto">
                        <div className={clsx(TABLE_MIN, 'w-full')}>
                            ...
                            {loading ? (
                                ...
                                <div className="px-5 py-10 text-center text-sm text-red-400 border-t border-red-500/20 bg-red-500/10">
                                    {loadError}
                                </div>
                            ) : builds.length === 0 ? (
                                ...
                                <div className="divide-y divide-surface-4">
                                    {builds.map(b => {
                                        const canDownload = b.artifact_available;
                                        return (
                                        <div
                                            key={b.build_id}
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-1 shrink-0">
                                                ...
                                                <Tooltip text={canDownload ? 'Download artifacts' : 'Artifacts available when the archive is ready'}>
                                                    <button
                                                        type="button"
                                                        onClick={() => {
