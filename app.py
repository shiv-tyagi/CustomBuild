...
def refresh_remotes():
    if os.environ.get('CBS_REMOTES_RELOAD_TOKEN') is None:
        app.logger.error("Couldn't retrieve authorization token from environment")
        return "Internal Server Error", 500

    token = request.get_json().get('token')
    if not token or token != os.environ.get('CBS_REMOTES_RELOAD_TOKEN'):
...
def generate():
    try:
        ...
        if chosen_version_info is None:
            raise Exception("Commit reference invalid ot not listed to be built for given vehicle for remote")
...
def get_deafults(vehicle_name, remote_name, commit_reference, board_name):
    ...
    # omit the last two elements as they always blank
