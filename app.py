...
build_metadata_manager = metadata_manager.BuildMetadataManager()
...
def generate():
    try:
        ...
        allowed_defines_at_commit = [
            feature.define
            for feature in ap_src_metadata_fetcher.get_build_options_at_commit(
                remote=chosen_remote,
                commit_ref=chosen_commit_reference
            )
        ]

        user_submitted_defines = [
            define
            for define in allowed_defines_at_commit
            if request.form.get(define) == '1'
        ]
