...
if ! [ -f "$REMOTES_JSON_PATH" ]; then
    ...
    python ../scripts/fetch_releases.py --basedir $CBS_BASEDIR
fi
...
if [ ! -d "$AP_REPO_METAGEN_PATH" ] || ! git -C "$AP_REPO_METAGEN_PATH" rev-parse --is-inside-work-tree > /dev/null; then
    ...
    git clone https://github.com/ardupilot/ardupilot.git "$AP_REPO_METAGEN_PATH" --no-recurse-submodules --origin ardupilot
fi
...
if [ "$CBS_ENABLE_INBUILT_BUILDER" -eq 1 ]; then
    ...
    if [ ! -d "$AP_REPO_BUILDGEN_PATH" ] || ! git -C "$AP_REPO_BUILDGEN_PATH" rev-parse --is-inside-work-tree > /dev/null; then
        ...
        git clone https://github.com/ardupilot/ardupilot.git "$AP_REPO_BUILDGEN_PATH" --no-recurse-submodules --origin ardupilot
    fi
fi
