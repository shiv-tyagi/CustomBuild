    ...
    for tag_info in tags:
        ...
        if matches:
            ...
            if v_num < IGNORE_VERSIONS_BEFORE:
                print("%s Version too old. Ignoring." % v_num)
                continue
...
parser.add_option("", "--appurl", type="string",
...
try:
    ...
    remotes = [remote for remote in remotes if remote['name'] != cmd_opts.remotename]
except Exception as e:
    print(e)
...
with open(cmd_opts.out, 'w') as f:
    ...
    print("Wrote %s" % cmd_opts.out)

# retrieve authentication token
token = os.environ.get('CBS_REMOTES_RELOAD_TOKEN')
# hit reload endpoint on custom build server app
if not token:
    print("Failed to retrieve authentication token for reload. Aborting.")
else:
