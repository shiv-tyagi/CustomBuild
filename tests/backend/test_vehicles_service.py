...
class TestVehiclesService:
    ...
    def test_get_versions_missing_version_number_uses_commit_ref(
        self, service, mock_versions_manager
    ):
        """Non-latest releases without version_number still appear, named by commit_ref."""
        mock_versions_manager.get_versions_for_vehicle.return_value = [
            VersionInfo(
                remote_info=RemoteInfo(
                    name="myfork",
                    url="https://github.com/example/ardupilot.git",
                ),
        ...
        assert len(versions) == 1
        assert versions[0].name == "abc123def456"
